 ...
 *
 * This version has been optimized for specific use-cases and is available at
 *
...
static FORCEINLINE int32_t atomic_cas_value(atomic32_t* dst, int32_t val, int32_t ref) { return _InterlockedCompareExchange(dst, val, ref) == ref ? 1 : 0; }
#ifdef ARCH_64BIT
...
static FORCEINLINE int  _try_acquire_lock(atomic32_t * lock) { return atomic_cas_value(lock, 1, 0); }
static FORCEINLINE void _acquire_lock(atomic32_t * lock) { while (!_try_acquire_lock(lock)); }
static FORCEINLINE void _release_lock(atomic32_t * lock) { while (!atomic_cas_value(lock, 0, 1)); }
...
//! Thread cache
typedef struct thread_cache_t thread_cache_t;
...
struct heap_t {
	...
	thread_cache_t span_cache[LARGE_CLASS_COUNT];
#endif
...
_memory_heap_cache_insert(heap_t* heap, span_t* span) {
	...
	if (_memory_span_list_push(&heap->span_cache[idx].span, span) <= (release_count * THREAD_CACHE_MULTIPLIER))
		return;
	heap->span_cache[idx].span = _memory_span_list_split(span, release_count);
...
_memory_heap_cache_extract(heap_t* heap, size_t span_count) {
	...
	for (++idx; idx < LARGE_CLASS_COUNT; ++idx) {
		if (heap->span_cache[idx].span) {
			span = _memory_span_list_pop(&heap->span_cache[idx].span);
			break;
		}
	}
	if (span) {
...
#if ENABLE_GREEDY_DEALLOC
		deallocated_size += _memory_size_class[span->size_class].size;
#endif
		_memory_deallocate_to_heap_core(heap, span, p);
		p = next;
	} while (p);
...
_memory_deallocate_defer(int32_t heap_id, void* p) {
	...
	int64_t deallocated_size = _memory_size_class[span->size_class].size;
#endif
...
_autotrim_start()
{
	...
	hQuit = CreateEventA(NULL, TRUE, FALSE, NULL);
	DWORD threadId;
	hThread =
		CreateThread(
		NULL,
...
rpmalloc_initialize_config(const rpmalloc_config_t* config) {
	...
	if (_memory_config.auto_trim_seconds)
		_autotrim_start();
		...
		while (heap) {
			...
			for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
				if (heap->span_cache[iclass].span)
					_memory_unmap_span_list(heap->span_cache[iclass].span);
			}
#endif
...
rpmalloc_thread_finalize(void) {
	...
	do {
		...
		orphan_counter = (uintptr_t)atomic_incr32(&_memory_orphan_counter);
		raw_heap = (heap_t*)((uintptr_t)heap | (orphan_counter & (uintptr_t)0xFF));
	}
